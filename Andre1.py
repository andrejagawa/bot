...
vipro = LINETCR.LINE()
#vipro.login(qr=True)
vipro.login(token='EtVS1msF8lF1sAbClPt9.3Wv+TySKVULbdXgG9fbjkq.Nj5xrdX8VFZuX54FBwt5Z1+jPRYzLoruIikQBFjc2mM=')
vipro.loginResult()
print "ANDRE MAH-Login Success\n\n=====[Sukses Login]====="
...
Bots=[mid]
Creator=["udf057fb58b24bee9410f11a0f4504929"]
admin=["udf057fb58b24bee9410f11a0f4504929"]
...
wait = {
    ...
    'timeline':True,
    "Timeline":True,
    "comment":"Bot Auto Like ©By : A⚷N🆒D⚕R🈷E\nContact Me : 👉 line.me/ti/p/~andrejagawa",    
    "commentOn":True,
    "commentBlack":{},
...
#                            Name = summon(op.param2)
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                     contact = vipro.getContact(msg.from_)
                     cName = contact.displayName
                     balas = ["Dont Tag!! Lagi Coli Dulu",cName + " Ngapain Ngetag,,,Sange Yacghh?",cName + " Tau Ajch Sama Cogan Tag Mele! Kalo Penting Langsung Pc Aja","Dia Lagi Off", cName + " Kenapa Tag Saya?","Dia Lagi Tidur\nJangan Di Tag " + cName, "Jangan Suka Tag Gua " + cName, "Kamu Siapa " + cName + "?", "Ada Perlu Apa " + cName + "?","Woii " + cName + " Jangan Ngetag, Riibut!"]
                     ret_ = random.choice(balas)
                     name = re.findall(r'@(\w+)', msg.text)
                     mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                     mentionees = mention['MENTIONEES']
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention2"] == True:          
                    contact = vipro.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["Sekali lagi nge tag gw sumpahin Gak Bisa Ngaceng Seumur Hidup!","Nggak Usah Tag-Tag! Kalo Penting Langsung Pc Aja","Woii " + cName + " Jangan Ngetag, Riibut!"]
                    ret_ = random.choice(balas)
                    name = re.findall(r'@(\w+)', msg.text)
                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                    mentionees = mention['MENTIONEES']
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention3"] == True:          
                    contact = vipro.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["Woii " + cName + ", Dasar Tukang Coli Ngetag Mulu!"]
                    balas1 = "Ini Foto Sii Tukang Coli Yang Suka Ngetag. . ."
                    ret_ = random.choice(balas)
                    image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus
                    name = re.findall(r'@(\w+)', msg.text)
                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
        ...
        if op.type == 25:
            ...
            if msg.text in ["Bot on"]:
                wait["Bot"] = True
                vipro.sendText(msg.to,"Bot Sudah Bisa Coli Kembali.")  
		...
		vipro.sendText(msg.to,"Itu Yang Buat Grup Ini")
            ...
            elif msg.text in ["Key group","Coliin group","Help group"]:
                vipro.sendText(msg.to,groupMessage)

            elif msg.text in ["Key","help","Help","Coliin"]:
                vipro.sendText(msg.to,helpMessage)

            elif msg.text in ["Key self","Coliin self","Help self"]:
                vipro.sendText(msg.to,selfMessage)

            elif msg.text in ["Key bot","Coliin bot","Help bot"]:
                vipro.sendText(msg.to,botMessage)

            elif msg.text in ["Key set","Coliin set","Help set"]:
                vipro.sendText(msg.to,setMessage)

            elif msg.text in ["Key media","Coliin media","Help media"]:
                vipro.sendText(msg.to,mediaMessage)
                
            elif msg.text in ["Key admin","Coliin admin","Help admin"]:
		...
		gid = vipro.getGroupIdsJoined()
            ...
            elif msg.text in ["Desah on","Autojoin on"]:
		if msg.from_ in admin:
                    wait["AutoJoin"] = True
                    wait["AutoJoinCancel"] = False
                    vipro.sendText(msg.to,"Auto Desah Sudah Aktif")
		else:
		    vipro.sendText(msg.to,"Khusus Admin")

            elif msg.text in ["Desah off","Autojoin off"]:
		if msg.from_ in admin:
                    wait["AutoJoin"] = False
                    vipro.sendText(msg.to,"Auto Desah Sudah Di Nonaktifkan")
		else:
		    vipro.sendText(msg.to,"Khusus Admin")
		...
		if msg.from_ in admin:
            ...
            elif msg.text in ["Coli1 on"]:
		if msg.from_ in admin:
                    wait["detectMention"] = True
                    wait["detectMention2"] = False
                    wait["detectMention3"] = False
                    wait["kickMention"] = False
                    vipro.sendText(msg.to,"Auto Coli1 Sudah Aktif")
		else:
		    vipro.sendText(msg.to,"Khusus Admin")

            elif msg.text in ["Coli1 off"]:
		if msg.from_ in admin:
                    wait["detectMention"] = False
                    vipro.sendText(msg.to,"Auto Coli1 Sudah Off")
		else:
		    vipro.sendText(msg.to,"Khusus Admin")	
		    
		    
            elif msg.text in ["Coli2 on"]:
		if msg.from_ in admin:
                    wait["detectMention"] = False
                    wait["detectMention2"] = True
                    wait["detectMention3"] = False
                    wait["kickMention"] = False
                    vipro.sendText(msg.to,"Auto Coli2 Sudah Aktif")
		else:
		    vipro.sendText(msg.to,"Khusus Admin")
            elif msg.text in ["Coli2 off"]:
		if msg.from_ in admin:
                    wait["detectMention2"] = False
                    vipro.sendText(msg.to,"Auto Coli2 Sudah Off")
		else:
		    vipro.sendText(msg.to,"Khusus Admin")	
		    

            elif msg.text in ["Coli3 on"]:
		if msg.from_ in admin:
                    wait["detectMention"] = False
                    wait["detectMention2"] = False
                    wait["detectMention3"] = True
                    wait["kickMention"] = False
                    vipro.sendText(msg.to,"Auto Coli3 Sudah Aktif")
		else:
		    vipro.sendText(msg.to,"Khusus Admin")

            elif msg.text in ["Coli3 off"]:
		if msg.from_ in admin:
                    wait["detectMention3"] = False
                    vipro.sendText(msg.to,"Auto Coli3 Sudah Off")
		else:
		    vipro.sendText(msg.to,"Khusus Admin")	
		...
		if msg.from_ in admin:
		    ...
		    vipro.sendText(msg.to,"Khusus Admin")
            ...
            elif msg.text in ["Roomcoli on"]:
                if wait["Sambutan"] == True:
                    if wait["lang"] == "JP":
                        vipro.sendText(msg.to,"Room Percolian Di Aktifkanヾ(*´∀｀*)ﾉ")
                else:
                    wait["Sambutan"] = True
                    if wait["lang"] == "JP":
                        vipro.sendText(msg.to,"Sudah Onヽ(´▽｀)/")

            elif msg.text in ["Roomcoli off"]:
                if wait["Sambutan"] == False:
                    if wait["lang"] == "JP":
                        vipro.sendText(msg.to,"Room Percolian Di Nonaktifkan(　＾∇＾)")
                else:
                    wait["Sambutan"] = False
                    if wait["lang"] == "JP":
                ...
                wait["Sider"] = True
                vipro.sendText(msg.to,"Siap On Cek Yang Suka CCTV")
                
            elif "Bintitan off" in msg.text:
                if msg.to in cctv['point']:
                    cctv['cyduk'][msg.to]=False
                    wait["Sider"] = False
                    vipro.sendText(msg.to, "Cek CCTV Off")
                else:
                    vipro.sendText(msg.to, "Heh Belom Di Set")                         


            elif msg.text in ["ROOM COLI"]:
                md = ""
		if wait["Sambutan"] == True: md+="╠➩✔️ Roomcoli : On\n"
		else:md+="╠➩❌ Roomcoli : Off\n"
		if wait["AutoJoin"] == True: md+="╠➩✔️ Auto Desah : On\n"
                else: md +="╠➩❌ Auto Desah : Off\n"
		...
		if wait["detectMention"] == True: md+="╠➩✔️ Auto Coli1 : On\n"
		else:md+="╠➩❌ Auto Coli1 : Off\n"		
		if wait["detectMention2"] == True: md+="╠➩✔️ Auto Coli2 : On\n"
		else:md+="╠➩❌ Auto Coli2 : Off\n"	
		if wait["detectMention3"] == True: md+="╠➩✔️ Auto Coli3 : On\n"
		else:md+="╠➩❌ Auto Coli3 : Off\n"			
		if wait["kickMention"] == True: md+="╠➩✔️ Auto Respon Kick : On\n"
		else:md+="╠➩❌ Auto Respon Kick : Off\n"				
		if wait["Sider"] == True: md+="╠➩✔️ Auto Sider : On\n"
		else:md+="╠➩❌ Auto Bintitan: Off\n"	
